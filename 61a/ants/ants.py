...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                self.ant = insect
            elif self.ant.can_contain(insect) and insect.is_container is False:
                self.ant.contain_ant(insect)
            elif insect.can_contain(self.ant) and self.ant.is_container is False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        count = 0
        while temp_place is not hive and temp_place is not None and count <= self.max_range:
            if len(temp_place.bees)!= 0 and count >= self.min_range:
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        if self.name == 'Impostor':
            self.reduce_armor(self.armor)

        ThrowerAnt.action(self,colony) # The queen ant throws a leaf

        # now doubles the damage of ants in her tunnel
        if self.place is not None:
            current = self.place.exit
            while current is not None:
                if current.ant: # if there is an ant in that place
                    if current.ant not in self.doubled:
                        current.ant.damage *= 2 # double the damage value
                        self.doubled.append(current.ant) # append that ant to doubled
