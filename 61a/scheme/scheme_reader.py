...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            val = scheme_read(src) #store the last term
            if src.current() == ')': #checking that only one element after dot
                src.remove_front()
                return val
            else:
                raise SyntaxError('should only have one element after dot')
            # END PROBLEM 2
        else:
            # BEGIN PROBLEM 1
