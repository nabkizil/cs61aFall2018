...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals.__len__() != vals.__len__():
            raise SchemeError()
        for _ in range(formals.__len__()):
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        new_operands = operands.map(lambda expr: scheme_eval(expr, env))
        return scheme_apply(self, new_operands, env)
        # END PROBLEM 5
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        func = do_lambda_form(Pair(target.second, expressions.second), env)
        target = target.first
        env.bindings[target] = func
        return str(target)
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        b = bindings.first
        if check_form(b, 2, 2):
            raise SchemeError('bad structure of bindings')
        if check_formals(Pair(b.first, nil)):
            raise SchemeError('invalid formals')
        formal = b.first
        val = scheme_eval(b.second.first, env)
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        check_form(expressions, 2, 2)
        # BEGIN PROBLEM 6
        env.bindings[target] = complete_eval(expressions.second.first, env)
        return str(target)
        # END PROBLEM 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
