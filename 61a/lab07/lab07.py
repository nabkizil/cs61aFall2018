...
def is_bst(t):
    ...
    def bst_max(t):
    	if len(t.branches):
    		return max(t.label, max([bst_max(branch) for branch in t.branches]))
    ...
    if len(t.branches) ==1:
    	...
    	first = bst_max(t.branches[0]) <= t.label and is_bst(t.branches[0])
    	second = bst_min(t.branches[1]) > t.label and is_bst(t.branches[1])
...
def in_order_traversal(t):
    ...
    if t.is_leaf():
    	...
    	for x in in_order_traversal(t.branches[0]):
    		yield x

    	yield t.label
    	for x in in_order_traversal(t.branches[1]):
    		yield x
